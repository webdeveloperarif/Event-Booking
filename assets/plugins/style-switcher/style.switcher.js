...
var styleSwitcher = {
  ...
  initialize: function () {
    ...
    $("head").append(
      $('<link rel="stylesheet">').attr(
        "href",
        "assets/plugins/style-switcher/style-switcher.css"
      )
    );
    $this.build();
    $this.events();
  ...
  build: function () {
    ...
    var switcher = $("<div />")
      ...
      .append(
        ...
        $("<div />")
          ...
          .append(
            $("<h5 />").html("Layout"),
            $("<ul />").addClass("options layout").attr("data-type", "layouts")
          )
          .append(
            $("<h5 />").html("Text Direction"),
            $("<ul />")
              .addClass("options direction")
              .attr("data-type", "directions")
          )
          .append(
            $("<h5 />").html("Pattern <small>(Boxed Layout)</small>"),
            $("<ul />")
    ...
    $.each(styleSwitcher.colors, function (i, value) {
      var color = $("<li />").append(
        $("<a />")
          .css("background-color", styleSwitcher.colors[i].Hex)
          .attr({
    ...
    $.each(layouts, function (i, value) {
      var layout = $("<li />").append(
        $("<a />")
          .css("background-color", layouts[i].Hex)
          .attr({
    ...
    $.each(directions, function (i, value) {
      var direction = $("<li />").append(
        $("<a />")
          .css("background-color", directions[i].Hex)
          .attr({
    ...
    $.each(patterns, function (i, value) {
      var pattern = $("<li />").append(
        $("<a />")
          .css({
            background:
              'url("assets/img/patterns/' +
              patterns[i].patternImage +
              "_" +
              $.cookie("skin") +
              '.png") repeat 50% 50%' +
              patterns[i].Hex,
  ...
  updatePatternColor: function () {
    ...
    var patternListToUpdate = this.container.find(
      "ul[data-type=patterns] li a"
    );
    var patternsBgHex = "";
    $(styleSwitcher.colors).each(function (i) {
      if ($.cookie("skin") == styleSwitcher.colors[i].colorName) {
